...
`tools/list` at `https://gateway.pipeworx.io/genius/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/genius_search \
  -H 'Content-Type: application/json' \
  -d '{"query":"humble kendrick lamar","limit":10}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/genius_search`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
